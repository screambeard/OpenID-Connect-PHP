...
class OpenIDConnectClient
{
    ...
    /**
     ...
     private function get_key_for_header($keys, $header) {
         foreach ($keys as $key) {
           if ($key->kty == 'RSA' || (!(isset($key->alg) && isset($header->kid)) && $key->kty == 'RSA') || ($key->alg == $header->alg && $key->kid == $header->kid)) {
                 return $key;
             }
         }
         if (isset($header->kid)) {
    ...
    private function verifyJWTclaims($claims) {
        ...
        return ((($claims->iss == $this->getProviderURL()) || (in_array($claims->iss, $this->extraIssuers)))
